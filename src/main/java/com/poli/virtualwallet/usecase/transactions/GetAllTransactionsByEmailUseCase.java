...
@Service
public class GetAllTransactionsByEmailUseCase {

    private ITransactionsRepository repository;
    private WalletMapper mapper;

    public GetAllTransactionsByEmailUseCase(ITransactionsRepository repository, WalletMapper mapper) {
    ...
    public Flux<TransactionsDTO> apply(String email) {
        return repository.findAll()
                .filter(transactions -> email.equals(transactions.getSource())
                                    || email.equals(transactions.getReceiver()))
